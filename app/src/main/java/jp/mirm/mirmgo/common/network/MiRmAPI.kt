...
object MiRmAPI {
    ...
    fun login(serverId: String, password: String): Boolean {
        ...
        val response = Http.post_X_WWW_URL_FORM_ENCODED(URLHolder.URL_AUTHENTICATE, mapOf(
        ...
        val title = document.getElementsByTag("title").`val`()
        return title == "MiRm | コントロールパネル"
    }
    ...
    fun sendCommand(command: String): Boolean {
        val json = Http.post(URLHolder.URL_SEND_COMMAND, mapOf("command" to command)) ?: throw MissingRequestException()
        val response = gson.fromJson(json, CommandResponse::class.java)
        return response.statusCode == 0
    }

    fun action(action: String): Boolean {
        val json = Http.post(URLHolder.URL_ACTION, mapOf("action" to action)) ?: throw MissingRequestException()
